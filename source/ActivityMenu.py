...
class ActivityMenu(QWidget):
    def __init__(self, motor_parameters):
        ...
        self.correction_label = None
        self.time_status_label = None
        self.current_training_length_label = None
        ...
        self.decrease_target_power_button = None
        self.increase_target_power_button = None
        self.current_target_power_label = None
        self.target_power_label = None
        self.measured_power_label = None
        ...
        self.init_ui(motor_parameters)

    def init_ui(self, current_parameters):
        self.logo_label = QtWidgets.QLabel(self)
        self.pixmap = QPixmap("image_400.jpg")  # Size of image can be modified if needed
        ...
        self.increase_target_power_button.setStyleSheet(
            "background-color: palegreen; border: 2 solid; border-radius: 1"
        )
        ...
        self.decrease_target_power_button.setStyleSheet(
            "background-color: palegreen; border: 2 solid; border-radius: 1"
        )
        ...
        # self.error_button = QtWidgets.QPushButton(self)  # TODO: Erase this part once error can be detected
        ...
        if self.startWatch:
            ...
            if int(self.counter / 10) < 10:
                ...
                if self.counter / 10 == 60.0:
                    self.second = "00"
                    self.counter = 0
                    mini = int(self.minute) + 1
                    if mini == self.MAX_TIME:
                        # self.close()
                        self.time_status_label.setText("Le temps d'entraînement visé est terminé")
                        self.time_status_label.adjustSize()
                    if mini < 10:
                        self.minute = "0" + str(mini)
                    else:
                        self.minute = str(mini)
    ...
    def update_labels(self, current_parameters):
        ...
        if current_parameters.get_current_power() > (
            current_parameters.get_target_power() * 1.1
        ):  # TODO: Change 25 by targeted speed
        ...
        elif current_parameters.get_current_power() < (
            current_parameters.get_target_power() * 0.9
        ):  # TODO: Change 25 by targeted speed
            self.measured_power_label.setStyleSheet("color: red")
