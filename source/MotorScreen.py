...
class MotorScreen(Screen):
    ...
    def manage_active_window(self, motor_parameters):
        ...
        if self.window_counter == 0:  # MainWindowMotor
            ...
            self.current_menu.show()
            # self.connect_buttons(self.current_menu)
            self.current_menu.increase_target_power_button.clicked.connect(
                lambda: self.event_function("increase_target_power")
            )
            self.current_menu.decrease_target_power_button.clicked.connect(
                lambda: self.event_function("decrease_target_power")
            )
            # self.current_menu.increase_training_length_button.clicked.connect(lambda : self.event_function(
            #     "increase_training_length"))
            # self.current_menu.decrease_training_length_button.clicked.connect(lambda : self.event_function(
            #     "decrease_training_length"))
            ...
            self.confirm_menu.confirmation_button.clicked.connect(
                lambda: self.confirmation_button_clicked(motor_parameters)
            )
